...
func Test_Should_Create_An_Event_Correctly(t *testing.T) {
	event, err := NewEvent("Concert", "Stadium", "Music Inc.", RatingFree, time.Now().Add(24*time.Hour), 100, 50.0, "http://x.jpg", 1)
	assert.Nil(t, err)
...
func Test_Should_Validate_The_Event_Correctly(t *testing.T) {
	event := &Event{
		Name:     "",
...
func Test_Should_Add_A_Spot_To_Event_Correctly(t *testing.T) {
	event, err := NewEvent("Concert", "Stadium", "Music Inc.", RatingFree, time.Now().Add(24*time.Hour), 100, 50.0, "http://x.jpg", 1)
	assert.Nil(t, err)
	assert.NotNil(t, event)
