...
func (r *mysqlEventRepository) FindEventByID(eventID string) (*domain.Event, error) {
	query := `
		SELECT 
			...
			ticket.price
		FROM events event
		LEFT JOIN spots spot 
			ON event.id = spot.event_id
		LEFT JOIN tickets ticket 
			ON spot.id = ticket.spot_id
		WHERE 
			event.id = ?
	`
	rows, err := r.db.Query(query, eventID)
	...
	for rows.Next() {
		...
		var eventID, eventName, eventLocation, eventOrganization, eventRating, eventImageURL sql.NullString
